...
CATEGORY_FIELDS = {
    'movies': ['released', 'redirect', 'started', 'finished', 'stars', 'star_link', 'imdb_link', 'perfect', 'opener: |'],
    'shows': ['released', 'redirect', 'started', 'finished', 'stars', 'star_link', 'imdb_link', 'perfect', 'opener: |'],
    'episodes': ['season', 'episode', 'show', 'show_link', 'released', 'redirect', 'score', 'imdb_link', 'perfect', 'opener: |'],
    'books': ['released', 'redirect', 'started', 'finished', 'stars', 'star_link', 'perfect', 'opener: |'],
    'albums': ['released', 'redirect', 'score', 'vinyl', 'vinyl_link', 'perfect'],
    'songs': ['album', 'album_link', 'released', 'redirect', 'score', 'vinyl', 'perfect'],
...
def clean_author(s):
    ...
    for sep in ['Photographs by', 'Photography by', 'Photos by',
                'Illustration by', 'Illustrated by', 'Video by']:
        if sep in s:
            s = s.split(sep)[0]
...
if args.category in SPOTIFY_CATEGORIES:
    ...
    if args.category == 'episodes':
        ...
        title = sanitize_title(ep_data.get('name', '') or og_title.split(' (')[0].strip())

        # Director as creator
        directors = ep_data.get('director', [])
        if isinstance(directors, list) and directors:
            creator = ', '.join(sanitize_title(d.get('name', '')) for d in directors if d.get('name'))
        elif isinstance(directors, dict):
            creator = sanitize_title(directors.get('name', 'Unknown'))
        else:
            creator = desc.split('Directed by ')[-1].split('.')[0].strip() if 'Directed by' in desc else 'Unknown'
        ...
        import re as _re
        for el in og['soup'].find_all(attrs={'data-testid': _re.compile('hero-subnav')}):
    ...
    if 'podcasts.apple.com' in host and '·' in og_desc:
        ...
        creator = sanitize_title(clean_author(raw_author)) if raw_author else sanitize_title(og['site_name'] or 'Unknown')
        publication = sanitize_title(og['site_name'] or '')
        if og['published']:
...
for field_entry in category_fields:
    ...
    if key in ('released', 'publication', 'published', 'season', 'episode', 'show', 'show_link', 'imdb_link') and f'\n{key}: ' in extra_field:
